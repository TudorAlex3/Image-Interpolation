function out = nn_2x2(f, STEP = 0.1)
    ...
    for i = 1 : n
        for j = 1 : n
            ...
            %calculez distanta dintre doua puncte (sunt 4 valori cunoscute)
            d1 = sqrt((x_int(i)-x1)^2+(y_int(j)-y1)^2);
            d2 = sqrt((x_int(i)-x1)^2+(y_int(j)-y2)^2);
            d3 = sqrt((x_int(i)-x2)^2+(y_int(j)-y1)^2);
            d4 = sqrt((x_int(i)-x2)^2+(y_int(j)-y2)^2);
            dist = [d1, d2, d3, d4];
            d = min(dist);
